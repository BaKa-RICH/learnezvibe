...
> 2026-08-20 第一组完成后追加口径（用户拍板）：**基础设施故障 run 不计入计量**（如 codex 账号欠费导致的空会话/截断 run，usage 为空或仅 1-3 条消息），在证据中单列披露；计量的墙钟/run 数/token 以成功 run 为准。此修订不改变任务正文、rubric、输入 revision、agent 配置与拓扑。
> 2026-08-20 第二组（Native-02 + OMAC-02）因实验环境共享性双样本污染（通道一：两臂共用同一 weekly agent 并发派发致上下文串扰；通道二：历史样本证据残留于工作区仓库被后续 agent 复用），**污染样本排除**（不计入计量、单列披露，同基础设施故障口径）。用户拍板（2026-08-20）：① 执行方式由“配对并发”修订为**同窗串行**——两臂同一环境窗口内先后运行，先跑完一臂（含全部三节点）再启动另一臂，保证同角色 agent 派发不重叠；② 正式样本槽**新开 Native-03、OMAC-03** 补足第二组；③ 重跑前清场：历史样本证据整体移出工作区仓库并推送远端。此修订发生在重跑样本开始前，符合不可变规则（不改变任务正文、rubric、输入 revision、agent 配置与拓扑）。

## 目标

在同一冻结输入、同一角色 agent 配置、同一 `collect -> write -> review` 业务拓扑和同一验收目标下，对比 Multica Native squad 编排与 OMAC 编排。每组取得 2 个有效正式样本，共 4 个有效样本。

正式样本槽固定为：`Native-01`、`OMAC-01`、`Native-02`、`OMAC-02`、`Native-03`、`OMAC-03`（后两个为 2026-08-20 第二组污染后新开补足，见修订记录）。执行方式为**同窗串行**（2026-08-20 由“配对并发”修订）：同一环境窗口内两臂先后运行，`Native-N` 全部三节点收敛后再启动 `OMAC-N`（或反之），保证同角色 agent 派发不重叠；禁止同一 arm 两样本并发或四样本全并发。`WEEK-9` 仅作为预演模板，不计入样本；已有取消 run 与污染 run 不计入样本。
